...
emit("ALFAK2_LAMBDA_E_GRID", field("lambda_e", "1"))
	...
	emit("ALFAK2_GRAPH_EDGE_WEIGHT", field("graph_edge_weight", "mutation"))
	anchor_ref <- field("anchor_count_reference", "")
	if (nzchar(anchor_ref) && is.finite(suppressWarnings(as.numeric(anchor_ref)))) {
	  emit("ALFAK2_ANCHOR_COUNT_REFERENCE", anchor_ref)
	  emit("ALFAK2_ANCHOR_COUNT_POWER", field("anchor_count_power", "1"))
	}
