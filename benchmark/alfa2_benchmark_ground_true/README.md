...
The Slurm script loads the configured R 4.4 module with `ml` before running `Rscript`. The default on this HPC is `R/4.4`. The R runner then requires `pkgload` and loads both packages with `pkgload::load_all()` from those source repositories. It validates the loaded namespace paths and fails instead of falling back to any `alfakR` installed in the active R module library.
